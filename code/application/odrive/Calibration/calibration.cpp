...
#include "FOC/foc.h"
#include "PreferenceWriter/PreferenceWriter.h"
...
void order_phases(PositionSensor *ps, ControllerStruct *controller, PreferenceWriter *prefs) {
    ...
    lwprintf_printf("Theta Start:   %f    Theta End:  %f\n\r", theta_start, theta_end);
    lwprintf_printf("Direction:  %d\n\r", direction);
    if (direction) { lwprintf_printf("Phasing correct\n\r"); } else if (!direction) {
        lwprintf_printf("Phasing incorrect.  Swapping phases V and W\n\r");
    }
...
void calibrate(PositionSensor *ps, ControllerStruct *controller, PreferenceWriter *prefs) {
    ...
    lwprintf_printf("Starting calibration procedure\n\r");
    float *error_f;
    float *error_b;
    int *lut;
    int *raw_f;
    ...
    const int n = 128 * NPP;
    // number of positions to be sampled per mechanical rotation.  Multiple of NPP for filtering reasons (see later)
    ...
    //dq0 transform on currents
    float current = sqrt(pow(controller->i_d, 2) + pow(controller->i_q, 2));
    lwprintf_printf(" Current Angle : Rotor Angle : Raw Encoder \n\r\n\r");
    for (int i = 0; i < n; i++) {
        // rotate forwards
        for (int j = 0; j < n2; j++) {
            theta_ref += delta;
    ...
    lwprintf_printf(" Sample Number : Lookup Index : Lookup Value\n\r\n\r");
    ...
    TIM1->CCR3 = (PWM_ARR >> 1) * (1.0f - 0.5); //Set duty cycles
    TIM1->CCR2 = (PWM_ARR >> 1) * (1.0f - 0.5);
    TIM1->CCR1 = (PWM_ARR >> 1) * (1.0f - 0.5);
