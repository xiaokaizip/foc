...
void zero_current(int *offset_1, int *offset_2, const int adc1_raw, const int adc2_raw) {
    // Measure zero-offset of the current sensors

    static int adc1_offset_sum = 0;
    static int adc2_offset_sum = 0;
    int n = 1024;
    for (int i = 0; i < n; i++) {
        ...
        delayus(.01);
        adc1_offset_sum += adc1_raw;
        adc2_offset_sum += adc2_raw;
    }
    *offset_1 = adc1_offset_sum / n;
    *offset_2 = adc2_offset_sum / n;
}
...
void commutate(ControllerStruct *controller, float theta) {
    ...
    // float q_th_in = (1.0f + .00393f * t_rise) * (controller->i_d * controller->i_d * R_PHASE * SQRT3 + controller->i_q *
    //                                              controller->i_q * R_PHASE * SQRT3);
    ...
    // controller->i_d_ref_filt = (1.0f - controller->alpha) * controller->i_d_ref_filt + controller->alpha *
    //                            controller->
    //                            i_d_ref;
    // controller->i_q_ref_filt = (1.0f - controller->alpha) * controller->i_q_ref_filt + controller->alpha *
    //                            controller->
    //                            i_q_ref;
    ...
    float v_d_ff = SQRT3 * (1.0f * controller->i_d_ref * R_PHASE - controller->dtheta_elec * L_Q * controller->i_q);
    //feed-forward voltages
    float v_q_ff = SQRT3 * (1.0f * controller->i_q_ref * R_PHASE + controller->dtheta_elec * (
                                L_D * controller->i_d + 1.0f * WB));

    // Integrate Error //
    controller->d_int += controller->k_d * controller->ki_d * i_d_error;
    controller->q_int += controller->k_q * controller->ki_q * i_q_error;

    controller->d_int = fmaxf(fminf(controller->d_int, OVERMODULATION * controller->v_bus),
                              -OVERMODULATION * controller->v_bus);
    controller->q_int = fmaxf(fminf(controller->q_int, OVERMODULATION * controller->v_bus),
                              -OVERMODULATION * controller->v_bus);
    ...
    limit_norm(&controller->v_d, &controller->v_q, OVERMODULATION * controller->v_bus);
    // Normalize voltage vector to lie within curcle of radius v_bus
    ...
    abc(controller->theta_elec + 0.0f * DT * controller->dtheta_elec, controller->v_d, controller->v_q,
...
void torque_control(ControllerStruct *controller) {
    float torque_ref = controller->kp * (controller->p_des - controller->theta_mech) + controller->t_ff + controller
                       ->kd * (controller->v_des - controller->dtheta_mech);
