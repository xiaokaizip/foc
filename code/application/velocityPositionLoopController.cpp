...
void velocityPositionLoopController::updata(float expect_velocity, float expect_position) {
     // // 读取编码器实际速度（假设 encoderData 全局可用）
     // float actual_velocity = encoderData.angular_velocity;
     //
     // // 速度误差
     // velocity_error = expect_velocity - actual_velocity;
     ...
     float position_error = expect_position - encoderData.cumulative_angle;
     Uq = position_error * position_kp;
     limit(&Uq, -1, 1);
     ...
     // // Park 反变换：DQ → AlphaBeta
     // U_alpha = Ud * FastCos(electric_angle) - Uq * FastSin(electric_angle);
     // U_beta = Ud * FastSin(electric_angle) + Uq * FastCos(electric_angle);
     //
     // // 手动 Clark 变换生成三相电压，并加上偏置
     // Ua = U_alpha + voltage_power_supply / 2.0f;
     // Ub = (sqrtf(3) * U_beta - U_alpha) / 2.0f + voltage_power_supply / 2.0f;
     // Uc = (-U_alpha - sqrtf(3) * U_beta) / 2.0f + voltage_power_supply / 2.0f;
