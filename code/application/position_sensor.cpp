...
void Encoder_Update(int devidx, float dt) {
    // 读取当前原始角度（0~16383）
    int raw_angle = AS5047_ReadData(devidx, ANGLECOM_AS5047P_VOL_REG_ADD);
    ...
    mod_mech_position = (float) raw_angle * MECH_ANGLE_PER_COUNT;
...
// 应用编码器安装方向
    ...
    float electrical_angle = fmodf((mod_mech_position - 2.0f) * POLE_PAIRS * ENCODER_DIRECTION, 2.0f * PI);
    if (electrical_angle < 0.0f) electrical_angle += 2.0f * PI;

    // ====== 4. 累积角度（度）======
    static float cumulative_angle_deg; // rad → deg
