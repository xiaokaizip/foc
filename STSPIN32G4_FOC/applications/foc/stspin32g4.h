...
#define STSPIN32G4_I2C_LOCKUSEPARANOID /**< Flag to check status register lock bit in functions STSPIN32G4_lockReg() \
                                          and STSPIN32G4_unlockReg() */
...
#define STSPIN32G4_HSI16 /**< Flag to automatically reduce system clock frequency before entering standby in function \
                            STSPIN32G4_standby() */
...
/**
@brief Configuration for VDS monitoring protection to be used with functions STSPIN32G4_setVDSP() and
STSPIN32G4_getVDSP().
@details The STSPIN32G4 embeds a circuitry which measures the voltage between the drain and the source of each MOSFET
(VDS) and compares it with a specified threshold set by voltage of SCREF pin. In case an overvoltage of one MOSFET is
detected, all gate driver outputs GHSx and GLSx go low whatever the driver inputs INHx and INLx. \n The protection
provides a deglitch filtering with configurable value. \n Triggering of the protection can be signaled via nFAULT pin.
...
/**
@brief Configuration for Thermal Shutdown signaling to be used with functions STSPIN32G4_setTHSD() and
STSPIN32G4_getTHSD().
...
@ingroup stspin32g4_Basic
  */
typedef struct {
  uint8_t vccUvlo: 1; /**< If 1 the device is in VCC UVLO condition. While in VCC UVLO condition the device cannot
                            drive MOSFETs*/
  uint8_t thsd: 1; /**< If 1 one voltage regulator is in Thermal Shutdown. While in Thermal Shutdown the device
                            cannot drive MOSFETs*/
  uint8_t vdsp: 1; /**< If 1 the VDS protection triggered. Use STSPIN32G4_clearFaults() to make the device operative
                          again. */
  uint8_t reset: 1; /**< If 1 the device performed a reset or power up. Use STSPIN32G4_clearFaults() to make the
                          device operative. */
...
@param [in] hdl Driver handler.
@note With flag STSPIN32G4_I2C_LOCKUSEPARANOID the lock bit of status register will be tested to 1 at the end of
locking.
@return HAL_ERROR in case of failure otherwise HAL_OK.
@ingroup stspin32g4_I2c
  */
...
@param [in] hdl Driver handler.
@note With flag STSPIN32G4_I2C_LOCKUSEPARANOID the lock bit of status register will be tested to 0 at the end of
unlocking.
@return HAL_ERROR in case of failure otherwise HAL_OK.
@ingroup stspin32g4_I2c
  */
